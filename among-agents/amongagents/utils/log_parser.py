...
class GameLogParser:
    ...
    def _validate_game_outcome(
        ...
        if (
            ...
            if final_impostor_count == 0:
                warnings.append(
                    f"Game claims 'Impostors win' but no impostors survived"
                )
            ...
            warnings.append(
                f"Game claims 'Tasks completed' but task completion tracking not implemented in parser"
            )
        ...
        if warnings:
            ...
            print(
                f"   This may indicate incomplete/malformed LLM outputs in the original game logs.\n",
                file=sys.stderr,
            )
